...
export default function EditarFrasePage() {
  ...
  useEffect(() => {
    async function carregarFrase() {
      ...
      if (error || !data) {
        setErro('Não foi possível carregar a frase.')
      ...
      setCategorySlug(data.category_slug || 'motivacao')
      setBackgroundId(data.background_id || 'bg-01')
      setFontFamily(data.font_family || 'serif-classica')
  ...
  try {
    ...
    if (updateError) {
      throw new Error('Erro ao salvar a URL da imagem.')
    }

    setSucesso('Imagem gerada com sucesso.')
  } catch (err) {
    setErro(err instanceof Error ? err.message : 'Erro inesperado ao gerar imagem.')
  } finally {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (error) {
        throw new Error('Erro ao atualizar a frase.')
      }

      setSucesso('Frase atualizada com sucesso.')
    ...
    return (
      <main style={pageStyle}>
        <div style={containerStyle}>
          <div style={statusBoxStyle}>Carregando frase...</div>
        </div>
      </main>
    )
  }
      ...
      <div style={containerStyle}>
        ...
        <section style={formWrapStyle}>
          <div style={{ marginBottom: '24px' }}>
            ...
            <h1 style={titleStyle}>Editar frase</h1>
            <p style={subtitleStyle}>
              Atualize a frase, categoria, fundo, estilo e dados de publicação.
            </p>
          </div>
          ...
          <div>
            <label style={labelStyle}>Imagem gerada</label>
            {imageUrlAtual ? (
              <img
                src={imageUrlAtual}
                alt="Imagem gerada da frase"
                style={{
                  width: '100%',
                  maxWidth: '320px',
                  borderRadius: '16px',
              ...
              <div style={statusBoxStyle}>Nenhuma imagem gerada ainda.</div>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
              ...
              >
                {gerandoImagem ? 'Gerando imagem...' : 'Gerar imagem'}
              </button>
